...
// Types pour correspondre exactement au backend
export interface BlogPost {
  id: number;
  title: string;
  slug: string;
...
const blogService = {
  ...
  getPosts: async (params: BlogPaginationParams = {}) => {
    try {
      const queryParams = new URLSearchParams();
      ...
      queryParams.append('sort_order', params.sort_order || 'newest');

      // const response = await axios.get<BlogResponse>(`${API_BASE_URL}/articles`, {
      //   params: {
      //     page: params.page || 1,
      ...
      const response = await axios.get<BlogResponse>(`${API_BASE_URL}/articles?${queryParams.toString()}`);
      ...
      //       per_page: 6,
      //       total_pages: 1,
      //       total_articles: blogPosts.length
      //     }
      //   }
      // };
    }
  ...
  getArticleDetails: async (slugOrId: string) => {
    try {
      const response = await axios.get<ArticleDetailsResponse>(
        `${API_BASE_URL}/articles/${slugOrId}`
      );
      return response.data;
    } catch (error) {
      console.error('Erreur lors de la récupération des détails de l\'article', error);
  ...
  searchArticles: async (query: string, page: number = 1, perPage: number = 6) => {
    try {
      const queryParams = new URLSearchParams();
      // Ajouter uniquement les paramètres définis
      ...
      // Filtrer les articles statiques
      ...
      return {
        ...
        data: {
          ...
          pagination: {
            ...
            per_page: perPage,
            total_pages: 1,
            total_articles: filteredPosts.length
          }
        }
      };
    }
